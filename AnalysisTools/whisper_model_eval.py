...
from auto_caption import transcribe_and_generate_srt_and_text
...
def transcribe_and_save_results(whisper_model_id, video_filename, directory, filename):
    logger.info(f"Transcribing {video_filename} using {whisper_model_id}...")
    plain_text_content, srt_content = transcribe_and_generate_srt_and_text(whisper_model_id, video_filename)
...
audio_file = "../Experiments"

logger = logging.getLogger("Whisper Eval Tool")
...
for video_file in get_video_files(args.file):
    ...
    baseline_result, _ = transcribe_and_save_results(whisper_model_ids[-1], video_file, dir_name, name)
    current_results[whisper_model_ids[-1]] = baseline_result

    for model_id in whisper_model_ids[:-1]:
        baseline_result, _ = transcribe_and_save_results(model_id, video_file, dir_name, name)
    ...
    for model_id in whisper_model_ids:
        ...
        logger.info(f"Similarity between {whisper_model_ids[-1]} and {model_id}: {similarity:.2f}")
